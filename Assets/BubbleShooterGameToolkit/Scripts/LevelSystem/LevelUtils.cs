...
namespace BubbleShooterGameToolkit.Scripts.LevelSystem
{
    public static class LevelUtils
    {
        ...
        public static Ball GenerateItem(int x, int y, LevelManager levelManager, LevelItem levelItem, Level level)
        {
            ...
            {
                // Use prefab references instead of direct name binding
                var prefabRef = PrefabReferences.Instance;
                if (levelItem.levelElement.Prefab.name == "1RandomBall" || 
                    (prefabRef != null && prefabRef.randomBall != null && levelItem.levelElement.Prefab.name == prefabRef.randomBall.name))
                {
            ...
            else
            {
                ...
                {
                    // Use prefab references for EmptyBall if available
                    var prefabRef = PrefabReferences.Instance;
                    if (prefabRef != null && prefabRef.emptyBall != null)
                    {
                        ball = CreateBall(prefabRef.emptyBall.name, x, y, levelItem);
                    }
                    else
                    {
